...
# where to save?
savedir = '/work/lm2612/GPprior/'
save_filename = 'optimised_kernel_randomdraws'
...
def run_one(name,output,X,y,names,lons,lats,lons1,lats1):
    ...
    path = '/work/lm2612/GPprior/coarse_prediction.csv'
    df = pd.read_csv(path)
    y_pred = df['y_pred'].values
    # rescale to be same magnitude
    mean_y = np.mean(d.y_test)
    sd_y = np.std(d.y_test)
    y_pred = (y_pred - np.mean(y_pred) )/np.std(y_pred)
    y_pred =  (y_pred * sd_y) + mean_y
    # interpolate
    y_pred = np.reshape(y_pred,(30,60))
    lats_coarse = np.linspace(-90.,90.,30)
    lons_coarse = np.linspace(0.,360.,60)
    f = interp2d(lons_coarse,lats_coarse,y_pred)
    y_pred_new = f(lons,lats)
    # reshape
    d.y_pred = y_pred_new.flatten().reshape(1,-1)
