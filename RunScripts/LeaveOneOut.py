...
alpha_list = np.concatenate((np.logspace(-16,-1,20),alpha_list))
...
X_types=X_types[1:2]
y_types=y_types[:1]
regions=[None,regions_many]
region_labels=['',str(len(regions_many))]
...
savedir=('/Users/lm2612/Documents/PhD/plots/RegressionPlots/MetricsCV%sLOO/'%no_of_cv)
    ...
    for i in range(len(X_types)):
        ...
        for j in range(len(y_types)):
            ...
            if type(X) is tuple:
                ...
                d = PredictionData(X,y,Names,lons,lats,lons1,lats)
            d.save_X()
